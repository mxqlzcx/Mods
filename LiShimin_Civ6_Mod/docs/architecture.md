...
└─────────────────────────────────────────────────────────────┘
          ...
          ▼           ▼           ▼           ▼
┌─────────────┐ ┌─────────┐ ┌──────────┐ ┌──────────┐
│Coronation.lua│ │HeroDeath│ │PrinceLine│ │  Utils   │
│  (登基仪式) │ │.lua     │ │.lua      │ │.lua      │
│              │ │(英雄死亡)│ │(藩王线)  │ │(工具函数)│
└─────────────┘ └─────────┘ └──────────┘ └──────────┘
```
...
Main.lua 中的监听器被调用
    ...
    └── 调用对应模块的处理函数
            │
            ├── Coronation.lua  → 登基仪式逻辑
            ├── HeroDeath.lua   → 死亡处理逻辑
            ├── PrinceLine.lua  → 藩王线逻辑
            │
            ▼
        UI 通知 / 游戏状态变更
```
...
| `config/` | `constants.lua` | 常量定义、枚举值 |
| `config/` | `version.lua` | 版本控制变量 |
| `src/Lua/` | `Main.lua` | 入口控制、状态管理 |
| `src/Lua/` | `EventBus.lua` | 事件调度中心 |
| `src/Lua/` | `Coronation.lua` | 登基仪式逻辑 |
| `src/Lua/` | `HeroDeath.lua` | 英雄死亡处理 |
